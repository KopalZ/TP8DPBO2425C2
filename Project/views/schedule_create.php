...
<head>
    ...
    <title>Tambah Jadwal - CRUD MVC</title>
...
<div class="container my-4">
    ...
    <div class="col-lg-6 m-auto">
        ...
        <?php endif; ?>
        <form method="post" action="<?= BASE_URL ?>/index.php?controller=schedule&action=create">
            <div class="card">
                <div class="card-header bg-primary"><h4 class="text-white text-center">Form Tambah Jadwal</h4></div><br>
                
                <label> DOSEN: </label>
                <select name="lecturer_id" class="form-control" required>
                    <option value="">-- Pilih Dosen --</option>
                    <?php 
                    // $lecturers datang dari ScheduleController
                    while($row = $lecturers->fetch_assoc()): ?>
                        <option value="<?= $row['id'] ?>" <?= (isset($this->schedule->lecturer_id) && $this->schedule->lecturer_id == $row['id']) ? 'selected' : '' ?>>
                            <?= $row['name'] ?> (<?= $row['nidn'] ?>)
                        </option>
                    <?php endwhile; ?>
                </select> <br>
                ...
                <select name="course_id" class="form-control" required>
                    ...
                    <?php 
                    // $courses datang dari ScheduleController
                    while($row = $courses->fetch_assoc()): ?>
                        <option value="<?= $row['id'] ?>" <?= (isset($this->schedule->course_id) && $this->schedule->course_id == $row['id']) ? 'selected' : '' ?>>
                ...
                <select name="day" class="form-control" required>
                    ...
                    <?php foreach ($days as $d): ?>
                        <option value="<?= $d ?>" <?= (isset($this->schedule->day) && $this->schedule->day == $d) ? 'selected' : '' ?>>
                            <?= $d ?>
                        </option>
                    <?php endforeach; ?>
                </select> <br>
